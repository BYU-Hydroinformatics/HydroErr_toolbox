function ret_value = spearman_r( sim, obs, remove_zero, remove_neg )
% Calculates the Spearman Rank Correlation Coefficient (R (Spearman)) for simulated and 
% observed data.
%   metric = spearman_r(sim, obs) Calculates the R (Spearman) error metric between the
%   simulated and observed data.
%
%   metric = spearman_r(sim, obs, remove_zero, remove_neg) Calculates the R (Spearman) 
...
switch nargin
    case 2
        ...
        % Computing the R (Spearman)
        ret_value = corr(sim, obs, 'Type', 'Spearman');
