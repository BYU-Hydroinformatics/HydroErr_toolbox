function ret_value = male( sim, obs, remove_zero, remove_neg )
% Calculates the Mean Absolute Log Error (MALE) for simulated and 
% observed data.
%   metric = male(sim, obs) Calculates the MALE error metric between the
%   simulated and observed data.
%
%   metric = male(sim, obs, remove_zero, remove_neg) Calculates the MALE 
...
switch nargin
    case 2
        ...
        sim_log = log1p(sim);
        obs_log = log1p(obs);
        ret_value = mean(abs(sim_log - obs_log));
