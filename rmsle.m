function ret_value = rmsle( sim, obs, remove_zero, remove_neg )
% Calculates the Root Mean Squared Log Error (RMSLE) for simulated and 
% observed data.
%   metric = rmsle(sim, obs) Calculates the RMSLE error metric between the
%   simulated and observed data.
%
%   metric = rmsle(sim, obs, remove_zero, remove_neg) Calculates the RMSLE 
...
switch nargin
    case 2
        ...
        % Computing the RMSLE
        ret_value = sqrt(mean(power(log1p(sim) - log1p(obs), 2)));
