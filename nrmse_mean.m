function ret_value = nrmse_mean( sim, obs, remove_zero, remove_neg )
% Calculates the Normalized Root Mean Square Error - Mean (NRMSE (Mean)) for simulated and 
% observed data.
%   metric = nrmse_mean(sim, obs) Calculates the NRMSE (Mean) error metric between the
%   simulated and observed data.
%
%   metric = nrmse_mean(sim, obs, remove_zero, remove_neg) Calculates the NRMSE (Mean) 
...
switch nargin
    case 2
        ...
        % Computing the NRMSE (Mean)
        rmse_value = sqrt(mean((sim - obs).^2));
        obs_mean = mean(obs);
        ret_value = rmse_value / obs_mean;
