function ret_value = msle( sim, obs, remove_zero, remove_neg )
% Calculates the Mean Squared Log Error (MSLE) for simulated and 
% observed data.
%   metric = msle(sim, obs) Calculates the MSLE error metric between the
%   simulated and observed data.
%
%   metric = msle(sim, obs, remove_zero, remove_neg) Calculates the MSLE 
...
switch nargin
    case 2
        ...
        sim_log = log1p(sim);
        obs_log = log1p(obs);
        ret_value = mean((sim_log - obs_log).^2);
