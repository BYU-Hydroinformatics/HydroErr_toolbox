function ret_value = h6_mhe( sim, obs, k, remove_zero, remove_neg )
% Calculates the Mean H6 Error (H6 (MHE)) for simulated and 
% observed data.
%   metric = h6_mhe(sim, obs) Calculates the H6 (MHE) error metric between the
%   simulated and observed data with k=1.
%
%   metric = h6_mhe(sim, obs, k) k is a float or integer input that
%   determines the power that the bottom term will be taken to. 
% 
%   metric = h6_mhe(sim, obs, k, remove_zero, remove_neg) The remove_zero
