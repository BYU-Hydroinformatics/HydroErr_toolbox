function ret_value = h6_mahe( sim, obs, k, remove_zero, remove_neg )
% Calculates the Mean Absolute H6 Error (H6 (MAHE)) for simulated and 
% observed data.
%   metric = h6_mahe(sim, obs) Calculates the H6 (MAHE) error metric between the simulated and observed data with k=1.
%
%   metric = h6_mahe(sim, obs, k) k is a float or integer input that
%   determines the power that the bottom term will be taken to. 
% 
%   metric = h6_mahe(sim, obs, k, remove_zero, remove_neg) The remove_zero
